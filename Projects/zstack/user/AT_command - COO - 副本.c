        ...
        if(AssoList[i].uiNwk_Addr!=0)
        {
            ...
            switch(AssoList[i].supportOD)
            {
                ...
                case 0x2BC1:
                    ...
                    break; 
                case 0x2BC2:
                    j += sprintf(sendBuffer+j+k,"%s,%x,%d,%d",buf,0x2BC2,DeviceStatus[i].status[0],DeviceStatus[i].status[1]);
                    break; 
                case 0x2BD7: 
                    j += sprintf(sendBuffer+j+k,"%s,%x,%d,%d,%d",buf,0x2BD7,DeviceStatus[i].status[0],DeviceStatus[i].status[1],DeviceStatus[i].status[2]);
                    break; 
                case 0x2904:
...
void _OnlineDeveice(getDeviceData Setting)
{
    ...
    if(Setting.RecSign == 1)
    {
        ...
        if(sign == 1)
        {
            sscanf(buffer,AT_Format,&buffer[0],&buffer[17],&buffer[18],&buffer[19]);
