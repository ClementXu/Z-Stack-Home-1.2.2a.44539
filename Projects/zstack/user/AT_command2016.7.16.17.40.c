...
void CheckDeviceStatus()
{
    ...
    for(i=0;i<5;i++)
    {
        if(DeviceStatus[k].uiNwk_Addr!=0 && AssoList[i].uiNwk_Addr !=0xffff)
        {
            ...
            switch(DeviceStatus[k].supportOD)
            {
                ...
                case 0x2BC1:
                  ...
                  clisterID = ZCL_CLUSTER_ID_GEN_ON_OFF;
                    break; 
                case 0x2BC2:
...
void _OnlineDeveice(uint8 *data)
{
    ...
    strcat(buffer,"+OK");
    osal_memcpy(&buffer[3],&data,sizeof(data));
    strcat(buffer,"\r\n");
...
//             //允许设备入网
            ...
            {  
                ...
                Setting.data[1] =(uint32 *)&buffer[17];
                Setting.data[2] =(uint32 *)&buffer[34];
                Setting.data[3] =(uint32 *)&buffer[35];
                Setting.data[4] =(uint32 *)&buffer[36];
                Setting.length = 5;
