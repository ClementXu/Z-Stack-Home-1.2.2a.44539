...
void _OnlineDeveice(uint8 *data)
{
    //1.查询mac地址是否在硬盘表中
    //2.如果存在，则将短地址设为目的地址
    //3.根据设备的OD来判断发送的指令
            ...
            switch(AssoList[i].supportOD)
            {
                ...
                case 0x2BC2:
                    if(data[29]==0)
                        ...
                        zclGeneral_SendOnOff_CmdOn( 1, &dstAddr, false, 0 );
                    zclGeneral_SendLevelControlMoveToLevel(1, &dstAddr,data[30],10,  false, 0);
                    break; 
                case 0x2BD7: 
                    if(data[29]==0)
                        zclGeneral_SendOnOff_CmdOff( 1, &dstAddr, false, 0 );
                        ...
                        zclGeneral_SendOnOff_CmdOn( 1, &dstAddr, false, 0 );
                    zclGeneral_SendLevelControlMoveToLevel(1, &dstAddr,data[30],10,  false, 0);
                    zclZHAtest_Light_Color_Status=((data[31] &0x00FF)<<8)+data[32];
...
//             //允许设备入网
            ...
            {  
                ...
                Setting.data[1] =(uint32 *)&buffer[12];
                Setting.data[2] =(uint32 *)&buffer[29];
                Setting.data[3] =(uint32 *)&buffer[30];
                Setting.data[4] =(uint32 *)&buffer[31];
                Setting.length = 5;
                Setting.type = VALUE_MIX;
                AnaDataProcess(&Setting);
                //CheckDeviceStatus();
                _OnlineDeveice(buffer);
                break;
            }
            case ENUM_AT_DEVICE:
            {
                ...
                for(j=0;j<6;j++)
                {
                    ...
                    { 
                        ...
                        if(NWK_command.NWK_General_Frame.NWKCG_CMDID[0]==0x20)
                        {
                            ...
                            {
                                BasicAttrsList.numAttr = 1;
                                BasicAttrsList.attrID[0] = ATTRID_SS_IAS_WD_MAXIMUM_DURATION;
                                //BasicAttrsList.attrID[1] = ATTRID_LEVEL_CURRENT_LEVEL;
                                zcl_SendRead( 1, &dstAddr,
                                ZCL_CLUSTER_ID_SS_IAS_WD, &BasicAttrsList,
                                ZCL_FRAME_CLIENT_SERVER_DIR, 0, 0);                   
