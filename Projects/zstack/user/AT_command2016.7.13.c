...
void sendDatatoComputer(getDeviceData *Setting)
{
    ...
    if(Setting->length==0)
    {
        sprintf(NWK_command,"+OK\r\n");
        HalUARTWrite(HAL_UART_PORT_0,&NWK_command,strlen(&NWK_command));
        return;
    }
    if(Setting->length==6)
    {
    ...
    if(Setting->type == VALUE_D)
    {
        ...
        for(i;i<Setting->length;i++)
        {
            ...
            j += sprintf(Setting->RecData+j+i,valueSign,*(Setting->data[i]));
        }
    }
    else
    {
        ...
        for(i;i<Setting->length;i++)
        {
            ...
            j += sprintf(Setting->RecData+j+i,valueSign,(char *)(Setting->data[i]));
        }        
                    ...
                    if(AssociatedDevList[j].shortAddr!=0 &&AssociatedDevList[j].shortAddr!=0xFFFF)
                    {
                        for(x=0;x<5;x++)
                        {
                            if(Node_Info[x].uiNwk_Addr==AssociatedDevList[j].shortAddr)
                            {
                                ...
                                osal_memcpy(Node_Info[x].showMAC,buf,sizeof(buf));
...
//
            case ENUM_AT_ACCEPTJOIN:
            {
                ...
                for(i=0;i<at_status.deviceCount;i++)
                {
                    for(j=0;j<at_status.deviceCount;j++)
                    {
                        if(strcmp(data[j],Node_Info[i].aucMAC)==0 & data[j]!=0)
                        {
                            flag=0;
                            for(k=0;k<5;k++)
                            {
                    ...
                    if(strcmp(Node_Info[i].aucMAC,data[j])!=0 && Node_Info[i].uiNwk_Addr!=0)
                    {
                            ...
                            if(NWK_command.NWK_General_Frame.NWK_AppDev_Frame.NWKCA_SUBIndex==0)
                            {
                                ...
                                {       
                                    ...
                                    //{
                                        ...
                                        {
                                            zb_ReadConfiguration( ZCD_NV_DEVICE_TABLE, sizeof(NODE_INFO_t)*6, &AssoList );
