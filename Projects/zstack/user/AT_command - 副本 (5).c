...
void sendDatatoComputer(getDeviceData *Setting)
{
    ...
    for(i;i<Setting->length;i++)
    {
        ...
        //sprintf(Setting->RecData,format,*(Setting->data[0]),*(Setting->data[1]));  
    }
	switch(Setting->length)
	{
		case 0:
			sprintf(Setting->RecData,"+OK");
			break;
		case 1:
			sprintf(Setting->RecData,format,*(Setting->data[0]));
			break;
		case 2:
			sprintf(Setting->RecData,format,*(Setting->data[0]),*(Setting->data[1]));
			break;
		case 3:
			sprintf(Setting->RecData,format,*(Setting->data[0]),*(Setting->data[1]),*(Setting->data[2]));
			break;	
		case 4:
			sprintf(Setting->RecData,format,*(Setting->data[0]),*(Setting->data[1]),*(Setting->data[2]),*(Setting->data[3]));
			break;	
		case 5:
			sprintf(Setting->RecData,format,*(Setting->data[0]),*(Setting->data[1]),*(Setting->data[2]),*(Setting->data[3]),*(Setting->data[4]));
			break;	
	}
	//sprintf(Setting->RecData,format,*(Setting->data[0]),*(Setting->data[1]),*(Setting->data[2]),*(Setting->data[3]));
    strcat(Setting->RecData,"\r\n");     
    //printf("%s",Setting->RecData);
    HalUARTWrite(HAL_UART_PORT_0,Setting->RecData,strlen(Setting->RecData));
...
{
	char SendBuffer[250];//发送数据的BUFFER
        uint8 i;
        char AT_Format[32]={0};
...
/////////////////////////////////////////////////////////
	...
	zclReadCmd_t BasicAttrsList;
        NLME_LeaveReq_t req;
        zb_ReadConfiguration(ZCD_NV_LOGICAL_TYPE, sizeof(uint8), &logicalType);
	//char RecSourceData[SOURCE_DATA_LENGTH];//接收原数据的缓存
	char AT_Format[]="AT+%[^=]";
	char AT_Name[10];//需要清零
	char NullString[]="";
	at_command Index;
        uint32 sendData[5];
	...
	switch(Index)
	{
         //获取MAC地址
            ...
            case ENUM_AT_LIGHT:
            {
                    ...
                    Setting.data[0] =(uint32 *)&zclZHAtest_OnOff;
                    Setting.length = 1;
                    Setting.type = VALUE_D;
                    AnaDataProcess(&Setting);
                    //sendDatatoComputer(&Setting);  
            ...
            case ENUM_AT_PIR:
            {
                ...
                dstAddr.endPoint=1;                  
                Setting.data[0] =(uint32 *)&zclZHAtest_Alarm_Status;
                Setting.length = 1;
                Setting.type = VALUE_D;
                AnaDataProcess(&Setting);
