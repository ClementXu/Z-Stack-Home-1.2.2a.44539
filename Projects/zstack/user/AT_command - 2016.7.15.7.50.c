...
void AT_Init()
{
    ...
    AT_GROUP[ENUM_AT_REMOVE]="REMOVE";  
...
void cleanNvTable()
{
    uint8 buffer[sizeof(NODE_INFO_t)*7];
    osal_memcpy(buffer,0,sizeof(NODE_INFO_t)*7);
    zb_WriteConfiguration( ZCD_NV_DEVICE_TABLE,(sizeof( NODE_INFO_t )*7), &buffer );
    NLME_UpdateNV(0x01);
...
void _AcceptJoin(uint8 *data)
{
    ...
    for(i=0;i<5;i++)
    {
            if(strcmp(data,p->device[i].showMAC)==0 & data[0]!=0)
            {
                
                for(k=0;k<5;k++)
                {
                    if(strcmp(AssoList[k].showMAC,data)!=0)
                        flag=0;
                    else if(strcmp(AssoList[k].showMAC,data)==0)
                    {
                        flag=2;
                        flag1=k;
                    }
                ...
                for(k=0;k<5;k++)
                {
                    if(AssoList[k].uiNwk_Addr==0 && flag==0)
                    {
                        osal_memcpy(&AssoList[k],&p->device[i],sizeof(NODE_INFO_t));
                        //zgDeviceNum ++;
                        osal_memset(&p->device[i],0,sizeof(NODE_INFO_t));
                        flag=1;
                        //k++;
                    }
                    else if(flag==2)
                    {
                        osal_memcpy(&AssoList[flag1],&p->device[i],sizeof(NODE_INFO_t));
                        osal_memset(&p->device[i],0,sizeof(NODE_INFO_t));
                        flag=1;                                    
                    }
                }
            }   
        if(strcmp(AssoList[k].showMAC,data)!=0 && p->device[i].uiNwk_Addr!=0)
        {
...
void _AddInfo()
{
    ...
    for(j=0;j<5;j++)
    {
        ...
        if(AssociatedDevList[j].shortAddr!=0 &&AssociatedDevList[j].shortAddr!=0xFFFF)
        {
            for(i=0;i<5;i++)
            {
                if(p->device[i].uiNwk_Addr==AssociatedDevList[j].shortAddr)
                {
                    ...
                    buf[16]=0;
                    osal_memcpy(p->device[i].showMAC,buf,Z_EXTADDR_LEN*2);
                    strcat( buffer,buf);
                    strcat( buffer,",");
                    osal_memcpy(data,zclZHAtest_ManufacturerName+1,16);
                    data[16]=0x00;
                    strcat( buffer,data);
                    strcat( buffer,",");
                    strcat( buffer,"00");
        ...
        if(AssociatedDevList[j].shortAddr!=0 &&AssociatedDevList[j].shortAddr!=0xFFFF)
        {
            for(i=0;i<5;i++)
            {
                if(p->device[i].uiNwk_Addr==AssociatedDevList[j].shortAddr)
                {
                    ...
                    //data1[16]=0x00;
                    strcat( buffer,p->device[i].showMAC);
                    strcat(buffer,",");
...
void _RemoveDevice(uint8 *data)
{
    ...
    for(i=0;i<5;i++)
    {
        if(strcmp(data,AssoList[i].showMAC)==0 & data!=0)
        {
...
void AT_process_event(char *recData,uint16 length)
{
	...
	switch(Index)
	{
         //获取MAC地址
            ...
            {
                uint8 buf[25];
                osal_memset(buf,0,25);
                if(logicalType!=0)
                {
                    sprintf(buf,"+OK=TESTDEVICE V1.8\r\n");
                    SendDatatoComputer(buf);  
                    //osal_memset(sendbuffer,0,16);
                }
                else if(logicalType==0)
                {
                    sprintf(buf,"+OK=GATEWAY V1.8\r\n");
                    SendDatatoComputer(buf);
                }
            break;
            }
            ...
            case ENUM_AT_FACTORY:
            {
                ...
                cleanNvTable();
//                uint8 buffer[5*sizeof(NODE_INFO_t)];
//                osal_memset(buffer,0,5*sizeof(NODE_INFO_t));
//                zb_ReadConfiguration( ZCD_NV_DEVICE_TABLE,(sizeof( NODE_INFO_t )*5), &buffer );
                Onboard_soft_reset();				
            break;
            }
            //重启设备（设备信息不丢失）              
...
//        {
            ...
            switch(IDX_CMD)
            {
                ...
                case DATE_TIME_Idx:
                    HalUARTWrite(HAL_UART_PORT_0,recData,sizeof(recData));
                break;
